...
def home(x_begin=0, x_over=500, y_begin=0, y_over=500, rock_lenght=40):
    ...
    # sd.circle(sd.get_point(x_begin, y_begin), 5)
    # sd.circle(sd.get_point(x_over, y_over), 5)
    ...
    sd.line(sd.get_point(x_begin, y_over),
            sd.get_point(x_begin + (x_over - x_begin) / 2, y_over + (x_over - x_begin) / 3), col_wal, hight)
    sd.line(sd.get_point(x_begin + (x_over - x_begin) / 2, y_over + (x_over - x_begin) / 3),
            sd.get_point(x_over, y_over), col_wal, hight)
    for y in range(y_begin, y_over, step_y):
        ...
        for x in range(x_begin + int(not (x_stat)) * step_x, x_over - int((step_x * int(x_stat)) / 2), step_x):
            wall_coord_x.append(x)
            point = sd.get_point(x + step_x * int(x_stat) / 2, y)
            point_end = sd.get_point(x + step_x * int(x_stat) / 2, y + step_y - 1)
        ...
        if y_win_begin < y < y_win_over:
            point_end_y = sd.get_point(win_coord_x[0] - step_x / 2, y)
            sd.line(sd.get_point(win_coord_x[-1] + step_x, y), sd.get_point(x_over, y), col_wal, hight)
        sd.line(point_y, point_end_y, col_wal, hight)
        x_stat = not (x_stat)
    x1, y1 = win_coord_x[0] - step_x / 2, win_coord_y[0]
    x2, y2 = win_coord_x[-1] + step_x, win_coord_y[-1] + step_y_const
